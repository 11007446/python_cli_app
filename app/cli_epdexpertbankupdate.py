...
def loadExcelData(filenamepath):
    ...
    with open(sqlfilenamepath + "专家信息检查脚本.sql", 'w', encoding='utf-8') as fw:
        fw.write('--执行该查询脚本若有专家联系电话与工作单位信息为空则需要重设专家ID')
        fw.write('\n')
        fw.write(
            "SELECT * FROM v_expert_batch_qry AS ebq WHERE ebq.batchgroupid = '{}'".
            format(batchgroupid))
        pass
